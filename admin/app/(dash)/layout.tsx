    ...
    >
        ...
        {NAV.map((item) => {
          ...
          return (
            ...
            >
              ...
              <span className={clsx('shrink-0 size-5', active ? 'text-indigo-600' : 'text-zinc-400 group-hover:text-zinc-700')}>
                {item.icon}
              </span>
      ...
      <div className="p-3">
        <div className="glass rounded-xl px-3 py-3 flex items-center gap-3">
          <div className="size-8 rounded-full gradient-primary text-white text-xs font-semibold flex items-center justify-center">
            {(me?.username?.[0] || 'A').toUpperCase()}
          </div>
          <div className="flex-1 min-w-0">
            <div className="text-sm font-medium truncate">{me?.username}</div>
            <button onClick={logout} className="text-xs text-zinc-500 hover:text-rose-600 transition">
              Выйти
            </button>
          </div>
          <Link href="/account" className="text-zinc-400 hover:text-ink transition" aria-label="Профиль">
        ...
        <header className="sticky top-0 z-30 sm:top-3 sm:mx-3 mt-0 sm:mt-3">
          <div className="glass sm:rounded-2xl px-4 sm:px-5 h-14 sm:h-14 flex items-center gap-3">
            ...
            <div className="ml-auto flex items-center gap-2">
              <Link
                href="/leads"
                className="hidden sm:inline-flex items-center gap-2 text-xs px-3 h-8 rounded-lg bg-white/60 hover:bg-white/85 transition text-ink"
                title="Заявки"
              >
                <I.Leads />
                <span className="text-zinc-700">Заявки</span>
              </Link>
              <div className="hidden sm:flex items-center gap-2 pl-3 ml-1 border-l border-zinc-200/60">
                <div className="size-7 rounded-full gradient-primary text-white text-[11px] font-semibold flex items-center justify-center">
                  {(me?.username?.[0] || 'A').toUpperCase()}
                </div>
                <span className="text-sm text-zinc-700">{me?.username}</span>
              </div>
            </div>
          </div>
        </header>
